...
void SceneOption::updateItemList() {
    items.clear();
    // Index 0: Section
    items.push_back({ "--- [ KEY CONFIG ] ---", "" }); 
    // Index 1-2
    items.push_back({ "[ PLAY KEY CONFIG ]", "11 Keys" });
    items.push_back({ "[ SYSTEM KEY CONFIG ]", "10 Keys" }); 
    ...
    // Index 3: Section
    items.push_back({ "--- [ PLAY SETTINGS ] ---", "" });
    // Index 4: GREEN NUMBER
    items.push_back({ "[ GREEN NUMBER ]", getValStr(std::to_string(Config::GREEN_NUMBER), 4) });
    // Index 5-6: LANE / SCRATCH WIDTH
    items.push_back({ "[ LANE WIDTH ]", getValStr(std::to_string(Config::LANE_WIDTH), 5) });
    items.push_back({ "[ SCRATCH WIDTH ]", getValStr(std::to_string(Config::SCRATCH_WIDTH), 6) });
    // Index 7: GAUGE DISPLAY
    items.push_back({ "[ GAUGE DISPLAY ]", getValStr((Config::GAUGE_DISPLAY_TYPE == 0 ? "1% STEP" : "2% STEP (IIDX)"), 7) });
    // Index 8: START UP SCREEN
    items.push_back({ "[ START UP SCREEN ]", getValStr((Config::START_UP_OPTION == 0 ? "TITLE" : "SELECT"), 8) });

    // Index 9: Section
    items.push_back({ "--- [ FOLDER SETTINGS ] ---", "" });
    // Index 10-15: 仮想フォルダ設定項目
    items.push_back({ "[ FOLDER: LEVEL ]", getValStr((Config::SHOW_LEVEL_FOLDER ? "ON" : "OFF"), 10) });
    items.push_back({ "[ FOLDER: LAMP ]", getValStr((Config::SHOW_LAMP_FOLDER ? "ON" : "OFF"), 11) });
    items.push_back({ "[ FOLDER: RANK ]", getValStr((Config::SHOW_RANK_FOLDER ? "ON" : "OFF"), 12) }); 
    items.push_back({ "[ FOLDER: TYPE ]", getValStr((Config::SHOW_CHART_TYPE_FOLDER ? "ON" : "OFF"), 13) });
    items.push_back({ "[ FOLDER: NOTES ]", getValStr((Config::SHOW_NOTES_RANGE_FOLDER ? "ON" : "OFF"), 14) });
    items.push_back({ "[ FOLDER: ALPHA ]", getValStr((Config::SHOW_ALPHA_FOLDER ? "ON" : "OFF"), 15) });
...
OptionState SceneOption::update(SDL_Renderer* ren, NoteRenderer& renderer) {
    ...
    if (state == OptionState::ADJUSTING_VALUE) {
        // SDL_GetKeyboardState または SDL_JoystickGetButton で状態を取得
        // 既存ロジック維持のため、イベントループ内でのタイマー更新を優先
    }
        ...
        if (e.type == SDL_JOYBUTTONDOWN) {
            ...
            if (state == OptionState::SELECTING_ITEM) {
                ...
                if (btn == Config::SYS_BTN_DECIDE) {
                    ...
                    else if ((cursor >= 4 && cursor <= 8) || (cursor >= 10 && cursor <= 15)) { state = OptionState::ADJUSTING_VALUE; updateItemList(); }
                ...
                if (cursor == 4) { // GREEN NUMBER
                ...
                else if (cursor == 6) { // SCRATCH WIDTH
                    ...
                    if (btn == Config::SYS_BTN_LEFT || btn == Config::SYS_BTN_RIGHT || btn == Config::SYS_BTN_UP || btn == Config::SYS_BTN_DOWN) {
                        if (cursor == 10) Config::SHOW_LEVEL_FOLDER = !Config::SHOW_LEVEL_FOLDER;
                        else if (cursor == 11) Config::SHOW_LAMP_FOLDER = !Config::SHOW_LAMP_FOLDER;
                        else if (cursor == 12) Config::SHOW_RANK_FOLDER = !Config::SHOW_RANK_FOLDER;
                        else if (cursor == 13) Config::SHOW_CHART_TYPE_FOLDER = !Config::SHOW_CHART_TYPE_FOLDER;
                        else if (cursor == 14) Config::SHOW_NOTES_RANGE_FOLDER = !Config::SHOW_NOTES_RANGE_FOLDER;
                        else if (cursor == 15) Config::SHOW_ALPHA_FOLDER = !Config::SHOW_ALPHA_FOLDER;
                    }
                    else if (btn == Config::SYS_BTN_DECIDE || btn == Config::SYS_BTN_BACK) state = OptionState::SELECTING_ITEM;
                    else changed = false;
                }
    ...
    if (state == OptionState::SELECTING_ITEM || (state == OptionState::ADJUSTING_VALUE && cursor != 5 && cursor != 6)) {
        ...
        if (state == OptionState::ADJUSTING_VALUE) {
            ...
            renderer.drawText(ren, (cursor == 4 ? "SYS-UP/DOWN: +-10  SYS-L/R: +-1" : "SYS-UDLR TO SWITCH TYPE"), 640, 670, {120, 120, 120, 255}, true, true);
        } else if ((cursor >= 4 && cursor <= 8) || (cursor >= 10 && cursor <= 15)) {
            renderer.drawText(ren, "PRESS DECIDE TO ADJUST", 640, 670, {150, 150, 150, 255}, true, true);
        }
    } 
    else if (state == OptionState::ADJUSTING_VALUE && (cursor == 5 || cursor == 6)) {
        ...
        for (int side = 0; side < 2; ++side) {
            ...
            for (int l = 0; l <= 8; ++l) {
                ...
                if (l < 8) {
                    ...
                    SDL_SetRenderDrawColor(ren, 80, 80, 80, 255); // 色を戻す
                }
            }
            // 判定線
